...
class Client {
  ...
  updateKeyIndexes({
    ...
    const { aliceSeed, secubitSeed } = this;
  ...
  async getTx() {
    ...
    const { alices, secubits, utxos } = await this.fetchAPI('/gettx', {
      method: 'POST',
      body: {
        uuid,
        fromAddress,
  ...
  async verifyRound({ round_id }) {
    ...
    try {
      ...
      // TODO: Enforce secubit index incrementing? Could cause issues with traditional wallets not displaying balances after 5 empty addresses in a row
      // if (this.roundState >= CLIENT_STATES.blind) {
      //   this.secubitIndex++; // Increment SecuBit address since the server may know it
      //   this.updateKeyIndexes({ changeIndex: this.changeIndex });
      // }
      if (this.callbackRoundComplete) this.callbackRoundComplete(finalTx);
    }
